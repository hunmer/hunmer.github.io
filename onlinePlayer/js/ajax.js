...
function ajaxSearch() {
    if(rem.wd === ""){
        layer.msg('検索内容を入力してください。', {anim:6});
        return false;
    }
    
    if(rem.loadPage == 1) { // 弹出搜索提示
        var tmpLoading = layer.msg('検索中', {icon: 16,shade: 0.01});
    }
    ...
    $.ajax({
        ...
        success: function(jsonData){
            ...
            // 调试信息输出
            if(mkPlayer.debug) {
                console.debug("検索結果数: " + jsonData.length);
            }
            ...
            if(rem.loadPage == 1)   // 加载第一页，清空列表
            {
                ...
                {
                    layer.msg('関連歌が見つかりませんでした。', {anim:6});
                    return false;
                }
                musicList[0].item = [];
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('検索結果の取得に失敗しました。 - ' + XMLHttpRequest.status);
...
function ajaxSong(music, callback)
{
    $.ajax({ 
        type: mkPlayer.method, 
        url: mkPlayer.api,
        data: "types=song&id=" + music.id + "&source=" + music.source,
        dataType : "jsonp",
        success: function(jsonData){
            // 调试信息输出
            if(jsonData.length > 0){
                jsonData[0].artist = Object.values(jsonData[0].artist).join(',');
                console.log(jsonData[0]);
                updateMinfo(jsonData[0]); // 更新音乐信息
                callback(jsonData[0]);
            }
            return true;
        },   //success
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('歌曲アドレスの取得に失敗しました。 - ' + XMLHttpRequest.status);
            console.error(XMLHttpRequest + textStatus + errorThrown);
        }   // error 
    }); //ajax
...
function ajaxPic(music, callback)
{
    ...
    $.ajax({ 
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('歌のカバーの取得に失敗しました。 - ' + XMLHttpRequest.status);
            console.error(XMLHttpRequest + textStatus + errorThrown);
        }   // error 
    }); //ajax
...
function ajaxPlayList(lid, id, callback) {
    ...
    $.ajax({
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('歌曲リストの読み込みに失敗しました。 - ' + XMLHttpRequest.status);
            console.error(XMLHttpRequest + textStatus + errorThrown);
            $(".sheet-item[data-no='" + id + "'] .sheet-name").html('<span style="color: #EA8383">歌曲リストの読み込みに失敗しました。</span>');     // 专辑名字
        }   // error  
    });//ajax
}
...
function ajaxLyric(music, callback) {
    ...
    $.ajax({
        ...
        success: function(jsonData){
            ...
            if (jsonData.lyric) {
                callback(music.id, music.source, jsonData.lyric, music.lyric_id, jsonData.tlyric);    // 回调函数
            } else {
                callback(music.id, music.source, '', music.lyric_id);    // 回调函数
            }
        },   //success
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('歌詞の読み込みに失敗しました - ' + XMLHttpRequest.status);
            console.error(XMLHttpRequest + textStatus + errorThrown);
...
{
    var tmpLoading = layer.msg('読み込み中...', {icon: 16,shade: 0.01});
    $.ajax({
        type: mkPlayer.method,
        url: mkPlayer.api,
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('歌曲リストの同期に失敗しました。 - ' + XMLHttpRequest.status);
            console.error(XMLHttpRequest + textStatus + errorThrown);
        }   // error
    });//ajax
    return true;
...
// ajax
// 参数 用户的网易云 id
function ajaxUserLogin(source, cellphone, password)
{
    var tmpLoading = layer.msg('読み込み中...', {icon: 16,shade: 0.01});
    $.ajax({
        type: mkPlayer.method,
        url: mkPlayer.api,
        data: "types=login&source=" + source+"&cellphone="+cellphone+"&password="+password,
        dataType : "jsonp",
        complete: function(XMLHttpRequest, textStatus) {
            if(tmpLoading) layer.close(tmpLoading);    // 关闭加载中动画
        },  // complete
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('失敗しました。 - ' + XMLHttpRequest.status);
            console.error(XMLHttpRequest + textStatus + errorThrown);
        }   // error
    });//ajax
    return true;
