...
switch($types)   // 根据请求的 Api，执行相应操作
{
	...
	case 'login':
		$cellphone = getParam('cellphone');
		$password = getParam('password');
        $data = $API->login($cellphone, $password);
        echojson($data);
		break;
    case 'url':   // 获取歌曲链接
        $id = getParam('id');  // 歌曲ID
    ...
    case 'play':
        header('Location: '.json_decode($API->format(true)->url(getParam('id')))->url);
        exit();
        break;
    ...
    case 'search_all':
        $s = urldecode(getParam('name'));  // 歌名
        $limit = getParam('count', 10);  // 每页显示数量
        $pages = getParam('pages', 1);  // 页码
        $r = [];
        //'netease', 
        foreach(array('tencent', 'xiami', 'kugou', 'baidu') as $server){
            $d = json_decode($API->site($server)->format(true)->search($s, [
                'page' => $pages, 
                'limit' => $limit
            ]), true);
              foreach($d as $v){
                if(strtolower($v['name']) == strtolower($s)){
                   $r[] = $v;
                }
              }
            }
...
function echojson($data)    //json和jsonp通用
{
  ...
  if(getParam('aplayer')){
        ...
        if(getParam('types') == 'lyric'){
            $data = $json['lyric'].$json['tlyric'];
        }else{
            foreach($json['playlist']['tracks'] as $v){
                $a = [];
                foreach($v['ar'] as $v_1){
                    $a[] = $v_1['name'];
                }
                ...
                $r[] = ['name' => $v['name'].$alia, 'artist' => $artist, 'url' => 'http://127.0.0.1/MKOnlineMusicPlayer-master/api.php?types=play&id='.$v['id'], 'cover' => $v['al']['picUrl'], 'lrc' => 'http://127.0.0.1/MKOnlineMusicPlayer-master/api.php?types=lyric&aplayer=true&id='.$v['id']];
            }
           $data = json_encode($r);
        }
    }
