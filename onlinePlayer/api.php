  ...
  if(getParam('aplayer')){
        ...
        if(getParam('types') == 'lyric'){
            ...
            foreach($json['playlist']['tracks'] as $v){
                ...
                $r[] = ['name' => $v['name'].$alia, 'artist' => $artist, 'url' => 'http://'.$_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF'].'?types=play&id='.$v['id'], 'cover' => $v['al']['picUrl'], 'lrc' => 'http://'.$_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF'].'?types=lyric&aplayer=true&id='.$v['id']];
            }
           $data = json_encode($r);
        }
    }
